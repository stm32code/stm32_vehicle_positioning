...
extern F32 longitude_sum, latitude_sum;
...
U32 addrShort2 = 0x0800C000 + 12 * 16; // 2

U32 addrShort3 = 0x0800C000 + 13 * 16; // 2

void W_Test(void)
{
    System_WriteInteriorFlash(addrShort1, (uint8_t *)&longitude_sum, sizeof(longitude_sum)); // 存入数据，函数内自动解锁，擦除，写入
    System_WriteInteriorFlash(addrShort2, (uint8_t *)&latitude_sum, sizeof(latitude_sum)); // 存入数据，函数内自动解锁，擦除，写入
		System_WriteInteriorFlash(addrShort3, (uint8_t *)&threshold_value_init.somg_value, sizeof(threshold_value_init.somg_value));
}
// 读取数据
void R_Test(void)
{
   
    System_ReadInteriorFlash(addrShort1, (uint8_t *)&longitude_sum, sizeof(longitude_sum)); // 读取数据，函数内自动按半字读取
    System_ReadInteriorFlash(addrShort2, (uint8_t *)&latitude_sum, sizeof(latitude_sum)); // 读取数据，函数内自动按半字读取
		System_ReadInteriorFlash(addrShort3, (uint8_t *)&threshold_value_init.somg_value, sizeof(threshold_value_init.somg_value));
		if(longitude_sum > 300 || longitude_sum < 0){
